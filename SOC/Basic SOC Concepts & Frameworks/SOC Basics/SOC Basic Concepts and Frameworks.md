...
4. Transport Layer  
	...
	1. TCP (Transmission Control Protocol) - ensuring reliability of delivering data between applications on host communications (IP Network). This is the transport layer of the OSI that is connection-oriented.  
    
	2. UDP (User Datagram Protocol) - use for time sensitive transmission (video playback or DNS lookup). This protocol is connectionless and does not guarantee delivery, order, or error checking. This is classified as a light-weight protocol.  
...
       II. Server Hello: Server responds with it chosen TLS version, cipher suites  with randoms numbers  
       III. Certificate: The server sends it digital certificate to the client for  authentication.  
       IV. Server Key Exchange: The server sends it public key or other key  exchange information 
       V. Client Key Exchange: The client generates a pre-master secret key,  encrypts it with the server's public key, and then sends it to the server.  
       VI. Change Cipher Spec: Both the client and server send a message  indicating that future messages will be encrypted.  
       VII. Complete: Both parties send a message to verify that the handshake  was successful, and the connection is secure.  
	...
	Steps: 
	 I. Encryption: SSL encrypt data to ensure privacy. If someone intercepts  the data, they will see only the jumble characters.  
	II. Authentication: SSL initiates an authentication process called a     handshake between two devices to confirm their identities.  
	III. Data Integrity: SSL digitally signs data to ensure it hasn’t been        tampered with, verifying that the data received is exactly what was sent by  the sender. 
	3. JPEG (Joint Photographic Experts Group): a committee that develops and maintains various digital images standards, including the widely used JPEG image compress format.  
	4. MPEG (Moving Pictures Experts Group) - an international organization that developed standards for compression, decompression and digital representation of audio and video data.  
...
7. Application Layer  
	...
	1. Data Representation - Ensure that transmitted data is in a format the receiving application can understand. 
	        I. Data Translation: Converts between formats (ASCII (American        Standard Code for Information Interchange) - EBCDIC (Extended Binary  Coded Decimal Interchange Code)) 

		 II. Character Encoding/Decoding: Uses standards UTF-8 bits (Unicode  Transfer Format) or Unicode
		 III. Data Compression: Reduces file size for faster transfer 
	     IV. Encryption/Decryption: Secure data 

	2. Network Services Access – Provides applications with access to network-based functions. 
	        I. Email Services: SMTP (Simple Mail Transfer Protocol) /Port 587 sends  emails. POP3 (Post Office Protocol) /Port 995 or non-secure Port 110 and  IMAP (Internet Message Access Protocol)/Port 993 incoming mails and  143 outgoing mails.   
		II. File Transfer: FTP (File Transfer Protocol) Port 21(sending  commands/managing between client and server)/20(data transfer port for  transferring files in active mode) TFTP (lightweight of FTP). 
		  III. Web Servers: HTTP (HyperTextTransferProtocol) port 80,        HTTPS(HyperTextTransferProtocolSecure) port 443. 
		  IV. Remote Access: Telnet(basic) SSH (Secure Shell) port 22. 

	3. Application Protocols – Define rules for communication between applications 
          I. File Transfer: FTP, TFTP 
         II. Web Communication: HTTP, HTTPS 
        III. Domain Name Resolution: DNS converts domain names to IP        addresses.  
        IV. Messaging: XMPP (Extensible and Message Presence Protocol) Port  80/443 is good for supporting a wide range of applications including        instant messaging and presence information and multipart chats. 

	4. Session Management – Manages and synchronizes communication sessions 
         I. Session Establishment and Termination: Handles login/logout processes 
        II. Synchronization: Uses checkpoints for recovery in large transfers. 
       III. Token Management: Prevents data collisions in half-duplex systems. 
       IV. Real-Time Communication: SIP (Session Setup), RTP (real-time        media delivery) 
...
- The DNS lookup process translates human-readable domain names into IP addresses within the network for communication.  
	...
	6. aching and Response 
		...
		I. Recursive Query: The client requests the resolver to handle the entire lookup process and return the final IP address.  
		II. Iterative Query: The resolver queries each DNS server in the hierarchy step-by-step, recieving referrals until the IP address is found.  
		III. Caching: Is critical in improving DNS performance. DNs server and client temporarily store resolved queries to reduce lookup times for subsequent requests. Cached records have a TTL (Time-To-Live) value, after which they expire and must be refreshed.  
...
9. Volumetric Attacks 
	...
	-  Aims to consume bandwidth and saturate teh network. The measure here is in bit per second(bps) which can cause attackes to scale to hundreds of gigabits or terabits per second. 
    

	      VII. DNS Amplification 
...
-       This is a TCP-based volumetric attack using RST (reset) and FIN         (finish) packets  
		...
		-       Since FIN and RST packets are used to gracefully and forcefully shut          down connections, flooding of them can confuse TCP stacks on          targeted system.   
...
5. RSA (Rivest-Shamir-Adleman) - leverages mathematical properties of prime numbers and modular arithmetic  
